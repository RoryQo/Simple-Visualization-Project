...
library(tidyr)
Percent <- Dailyshowguests %>% group_by(YEAR) %>% count(YEAR) %>% mutate(Percent = n / sum(n)*100)
...
* We need to pair the percents of the AMC vector to years in order to graph the line  
  ...
  + Create the data frame that the Acting, Comedy, and Music line will be created from with the as.data.frame()function
  
    + Make each vector (AMC and year) a column in the arguments of the as.data.frame function, so a year is paired with the percent for that corresponding year.
...
* We need to pair the percents of the Gov vector to years in order to graph the line  
  ...
  + Create the data frame that the Government and Politics line will be created from with the as.data.frame()function
  
    + Make each vector a column(year, Gov) in the arguments of the as.data.frame function, so a year is paired with the percent for that corresponding year.
